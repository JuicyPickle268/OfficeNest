"""
DeepSeek LLM 客户端 — 实现 ILLMClient 接口，OpenAI SDK 调用。

支持：
    - 流式输出 (chat_stream)：逐 token 回调 thinking/content/tool
    - 思考模式：DeepSeek reasoning_content → [思考]...[/思考] 包裹
    - 热切换：_model / _base_url / _api_key 可运行时修改（模型下拉）
"""
import json
...
class DeepSeekClient(ILLMClient):
    """OpenAI 兼容客户端。支持 DeepSeek / GLM / 商汤等所有 OpenAI-compatible API。"""
