            ...
            for at_name, sid, agent_msg in agents_info:
                try:
                    result = loop.run_until_complete(
                        self._app.process(agent_msg, session_id=sid, user_id=self._current_user)
                    )
    ...
    def _run_agent(self, text: str, session_id: str):
        ...
        def _run():
            ...
            try:
                ...
                result = loop.run_until_complete(
                    self._app.process(text, on_token=on_token, pinned_file=self._pinned_file, session_id=session_id, user_id=self._current_user)
                )
