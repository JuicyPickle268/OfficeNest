...
skills/              LLM 可调用工具（60+ 个，12 个模块）
  excel_skill.py     Excel 操作（10 工具）
  word_skill.py      Word 操作（8 工具 + 2 联动）
  vision_skill.py    视觉/PDF（8 工具）
  system_skill.py    文件管理 + 搜索（13 工具）
...
panel_qt._send_command("帮我查合同表")
    → MotherApp.process(user_input, session_id)
        → 加载历史 / 文件列表 / Excel 摘要
        → engine.process(user_message, chat_history, on_token)
...
2. **vision_client 走 httpx 异步**——GLM 商汤双 provider，不再依赖 zai-sdk
...
- `panel/admin_panel.py`（tkinter 面板）
- `skills/improvement_skill.py`
- `adapters/improvement_tracker.py`
- `adapters/word_renderer.py`
- `adapters/prompt_optimizer.py`
- `api_server.py`
- `web/` 目录
- `reflex/` 相关
- `user_store.py` / `user_skill.py`（用户系统回滚，待重新设计）

## 当前状态（v2.2 — 2026-07-28）

- **60+ 个工具**，涵盖 Excel/Word/PDF/VBA/PowerShell/搜索/知识库/工作流
- **多模型热切换**：运行时切换 provider/key/base_url
- **多 Agent 群聊**：@ 会话协作，共享日志 + 各自历史并行回复
- **5 标签页 Qt 面板**：仪表盘 + 日志 + 用户 + 知识库 + 设置，侧边栏会话管理
- **知识库**：ChromaDB 本地嵌入 + SQLite 注册表
- **视觉**：GLM + 商汤双 provider，httpx 异步，自动降级
