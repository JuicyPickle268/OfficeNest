"""
视觉 Skill：截图 Excel/Word → GLM 分析布局、格式、图表。
"""
import base64
import io
...
class VisionSkill(BaseSkill):
    ...
    def __init__(self, glm_api_key: str = "", glm_model: str = "glm-4.6v-flash"):
        self._glm_key = glm_api_key
    ...
    def vision_analyze(self, image_b64: str, prompt: str) -> str:
        """调 GLM 分析截图。"""
        if not self._glm_key:
            # 降级：纯文本描述
            return ("（视觉分析不可用：未配置 GLM API Key。"
                    "请在设置中填写智谱 API Key。"
                    f"当前只能看到 base64 前100字符: {image_b64[:100]}...）")
        try:
            from adapters.llm.glm_vision import GLMVisionClient
            client = GLMVisionClient(self._glm_key, self._glm_model)
            return client.analyze(image_b64, prompt)
    ...
    def pdf_analyze(self, filepath: str, prompt: str = "", page: int = 0) -> str:
        """PDF 页面 → 图片 → GLM 视觉分析。"""
        if not self._glm_key:
            return "❌ 需要 GLM API Key"
        path = Path(filepath)
        if not path.is_absolute():
            for d in ["./workbooks", "./output", "."]:
                p = Path(d) / path.name
        ...
        try:
            ...
            doc.close()
            return self.vision_analyze(img_b64, prompt or f"分析这个PDF页面的内容和布局")
        except Exception as e:
            return f"❌ PDF分析失败: {e}"
