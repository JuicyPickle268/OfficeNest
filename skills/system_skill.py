...
class SystemSkill(BaseSkill):
    ...
    def get_tools(self) -> list[dict]:
        tools = [
            ...
            {"name": "zhihu_hotlist", "fn": self.zhihu_hotlist,
             "schema": self._schema("获取知乎当前热榜", {
                 "limit": {"type": "integer", "description": "返回数量，默认10，最大30"},
             }, [])},
            {"name": "zhihu_favlists", "fn": self.zhihu_favlists,
             "schema": self._schema("获取知乎收藏夹列表", {}, [])},
            {"name": "zhihu_favlist", "fn": self.zhihu_favlist,
             "schema": self._schema("获取收藏夹中的内容（支持分页）", {
                 "url_token": {"type": "integer", "description": "收藏夹 URL Token"},
                 "offset": {"type": "integer", "description": "分页偏移，0=前20条，20=21-40条"},
    ...
    def zhihu_hotlist(self, limit: int = 10) -> str:
        """获取知乎热榜。"""
        if not self._zhihu_key:
            return "❌ 需要知乎 API Key"
        import httpx, time
        try:
            ...
            data = resp.json()
            if data.get("Code") != 0:
                return f"热榜获取失败: {data.get('Message', '')}"
            items = data["Data"]["Items"]
            if not items:
                return "（热榜为空）"
            lines = [f"🔥 知乎热榜 TOP{len(items)}:"]
    ...
    def zhihu_favlists(self) -> str:
        ...
        try:
            resp = httpx.get(
                "https://developer.zhihu.com/api/v1/user/favlists",
                params={"Limit": 20},
                headers={
                    "Authorization": f"Bearer {self._zhihu_key}",
                    "X-Request-Timestamp": str(int(time.time())),
            ...
            data = resp.json()
            if data.get("Code") != 0:
                return f"收藏夹获取失败: {data.get('Message', '')}"
            items = data["Data"]["Items"]
            if not items:
                return "（没有收藏夹）"
            lines = []
            for it in items:
                public = "🔓" if it.get("IsPublic") else "🔒"
                lines.append(f"  {public} [{it['UrlToken']}] {it['Title']}")
                if it.get("Description"):
                    lines.append(f"     {it['Description'][:60]}")
            return "\n".join(lines)
        except Exception as e:
            return f"收藏夹获取失败: {e}"

    def zhihu_favlist(self, url_token: int, offset: int = 0, limit: int = 20) -> str:
        """获取收藏夹内容（支持分页）。"""
        if not self._zhihu_key:
            return "❌ 需要知乎 API Key"
        import httpx, time
        try:
            resp = httpx.get(
                "https://developer.zhihu.com/api/v1/user/favlist_contents",
                params={"FavlistUrlToken": url_token, "Offset": offset, "Limit": min(limit, 20)},
                headers={
                    "Authorization": f"Bearer {self._zhihu_key}",
                    "X-Request-Timestamp": str(int(time.time())),
                },
            ...
            data = resp.json()
            if data.get("Code") != 0:
                return f"收藏夹内容获取失败: {data.get('Message', '')}"
            items = data["Data"]["Items"]
            if not items:
                return "（已到末尾）"
            lines = [f"📋 第{offset+1}-{offset+len(items)}条:"]
            for i, it in enumerate(items):
                lines.append(f"{offset+i+1}. {it['Title']}")
                if it.get("Summary"):
                    lines.append(f"   {it['Summary'][:120]}")
                lines.append(f"   👍{it.get('LikeCount',0)} | {it['Url']}")
            paging = data["Data"].get("Paging", {})
            if not paging.get("IsEnd", True):
                next_offset = int(paging.get("NextOffset", offset + limit))
                lines.append(f"（共{paging.get('Totals',0)}条，下一页 offset={next_offset}）")
            return "\n".join(lines)
        except Exception as e:
            return f"收藏夹内容获取失败: {e}"
